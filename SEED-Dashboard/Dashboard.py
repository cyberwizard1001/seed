...
def fetch_FoodGroups():
    ...
    try:
        ...
        food_df = pd.DataFrame.from_dict({0:['No data']})
        food_df.to_csv('data/FoodGroups.csv', index=False, header=['Food Groups'])
...
def fetch_Anthropometric():
    ...
    try:
        ...
        food_df = pd.DataFrame.from_dict({0:['No data']})
        food_df.to_csv('data/AnthropometricParameters.csv', index=False, header=['Anthropometric Parameters'])
...
def fetch_Biochemical():
    ...
    try:
        ...
        food_df = pd.DataFrame.from_dict({0:['No data']})
        food_df.to_csv('data/BiochemicalParameters.csv', index=False, header=['Biochemical Parameters'])
...
def fetch_Clinical():
    ...
    try:
        food_df = pd.DataFrame.from_dict(data_dict)
        food_df.to_csv('data/ClinicalParameters.csv', index=False, header=['Aadhaar', 'Month', 'NeckPatches', 'PaleSkin', 'Pellagra', 'WrinkledSkin', 'TeethDiscolouration', 'BleedingGums', 'Cavity', 'WeakGums', 'AngularCuts', 'InflammedTongue', 'LipCuts', 'MouthUlcer', 'BitotSpot', 'Xeropthalmia', 'RedEyes', 'Catract','HairFall', 'DamagedHair', 'SplitEnds', 'Discolouration', 'DarkLines', 'SpoonShapedNails', 'BrokenNails', 'PaleNails','LeanBodyMass','BonyAppearance','Goitre','Obesity'])
    except:
        food_df = pd.DataFrame.from_dict({0:['No data']})
        food_df.to_csv('data/ClinicalParameters.csv', index=False, header=['Clinical Parameters'])
